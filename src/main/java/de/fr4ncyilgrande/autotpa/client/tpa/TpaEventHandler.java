...
public final class TpaEventHandler {
    ...
    private boolean handle(Component messageText) {
        ...
        if (!match.rule.isRegularTpaRequest()) {
            if (this.config.debugMode) {
                LOGGER.info("[AutoTPA] Ignoring /tpahere request from {}", playerName);
            }
            return true;
        }

        if (this.config.shouldAutoAccept(playerName)) {
            if (this.acceptor.tryAccept(playerName, match.rule, this.config)) {
                LOGGER.info("[AutoTPA] Auto-accepted TPA request from {}", playerName);
            }
    ...
    private Match findMatch(String message) {
        ...
        for (PatternRule rule : this.config.patterns) {
            ...
            if (matcher.find()) {
                String name = rule.extractPlayerName(matcher);
                if (ModConfig.isValidPlayerName(name)) {
